%  res = relja_simplenn(net, x, dzdy, res, varargin)
%
%  Author: Relja Arandjelovic (relja@relja.info)
%
%  Relja Arandjelovic modified the function found in matconvnet
%  to include the option to skip the forward pass (skipForward)
%  if res from a previous forward pass is provided.
%  Also added conserveMemoryReLU, conserveMemoryDepth, ..
%
%
...
function res = yul_simplenn(net, x, dzdy, res, varargin)

opts.res = [] ;
opts.conserveMemory = false ;
opts.conserveMemoryReLU= true;
opts.sync = false ;
...
opts.conserveMemoryDepth= false;
opts.skipForward= false;

opts = vl_argparse(opts, varargin);

n = numel(net.layers) ;
lastBackpropLayer= max(1, n-opts.backPropDepth+1);
...
if ~doder && opts.skipForward
    error('Can''t do forward pass with skipForward==true');
end
...
if nargin <= 3 || isempty(res)
  if opts.skipForward, error('Can''t skip forward, need the "res" argument'); end
  res = struct(...
    'x', cell(1,n+1), ...
    'dzdx', cell(1,n+1), ...
    'dzdw', cell(1,n+1), ...
...
for i=1:n
  ...
  switch l.type
    ...
    case 'pool'
      res(i+1).x = vl_nnpool(res(i).x, l.pool, ...
                             ...
                             cudnn{:}) ;
    case 'put_n_2_h'
      res(i+1).x = yul_put_n_2_h(res(i).x);
    case 'normalize'
  ...
  forget = opts.conserveMemory || (opts.conserveMemoryDepth && i<lastBackpropLayer) || (opts.conserveMemoryReLU && strcmp(l.type, 'relu'));
